...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 修改用户信息
            async editdetail() {
                const result = await this.$http.put("/dataArea/infoModify", this.form1,{
                    headers: {
                        Authorization: "Bearer " + JSON.parse(localStorage.getItem('userdata')).token
                    }
                });
            ...
            deteleUser(area) {
                this.$confirm("此操作将永久删除, 是否继续?", "提示", {
                    ...
                    .then(async () => {
                        const result = await this.$http.delete("/dataArea/delete?daId=" + area.daId,{
                            headers: {
                                Authorization: "Bearer " + JSON.parse(localStorage.getItem('userdata')).token
                            }
                        });
            ...
            deleteBatch(){
                let ids = this.multipleSelection.map(v=>v.daId);
                this.$confirm("此操作将永久删除, 是否继续?", "提示", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning",
                    ...
                    .then(async () => {
                        const result = await this.$http.post("/dataArea/deletebatch" , ids ,{
                            headers: {
                                Authorization: "Bearer " + JSON.parse(localStorage.getItem('userdata')).token
                            }
                        });
